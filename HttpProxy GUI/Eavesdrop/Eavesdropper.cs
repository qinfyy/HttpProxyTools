...
namespace Eavesdrop
{
    public static class Eavesdropper
    {
        ...
        private static async Task OnRequestInterceptedAsync(RequestInterceptedEventArgs e)
        {
            ...
            {
                await interceptedTask.ConfigureAwait(false);
            }
        }
        ...
        public static List<string> Overrides { get; } = new List<string>();
        ...
        public static void Initiate(int port, Interceptors interceptors, bool setSystemProxy)
        {
            lock (_stateLock)
            {
                ...
                Task.Factory.StartNew(InterceptRequestAsync, TaskCreationOptions.LongRunning).ConfigureAwait(false);
                if (setSystemProxy)
                {
